...
public class ProductService {
	...
	public int createProduct(String id, JSONObject json) throws ParseException {
		...
		String sql = "insert into s_product(id,lb_id,kc,price,product_name,product_description,create_time) values(?,?,?,?,?,?,?)";
	...
	public int EditProduct(String id, JSONObject json) {
		...
		BigDecimal price = json.getBigDecimal("price");
		BigDecimal kc = json.getBigDecimal("kc");
		String sql = "update s_product set lb_id=?,price=?,kc=?,product_name=?,product_description=?,update_time=? where id=?";
	...
	public Map getProductById(String productId) {
		String sql = "select t.* from s_product t where t.delete_flag='0' and t.id=?";
		Map map = jdbcService
	...
	public List getProductByPage(int page, int size) {
		String sql = "select * from (select t1.*,t2.name as lbZw from s_product t1  left join s_lb t2 on t1.lb_id=t2.id where t1.delete_flag='0' order by t1.create_time desc) t limit ?,?";
		Object[] args = new Object[] { (page - 1) * size, size };
		return jdbcService.queryForList(sql, args);
	}

	public int getProductCount() {
		String sql = "select count(*) from s_product where delete_flag='0'";
		Object[] args = new Object[] {};
		return jdbcService.count(sql, args);
	}

	public int deleteProduct(String id) {
		String sql = "update s_product set delete_flag='1' where id=?";
