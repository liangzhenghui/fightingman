...
<!--         <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="uploadImg()">继续上传产品图片</a> -->
	...
	<table id="productList" class="easyui-datagrid" 
		...
		<thead>
			<tr>
				<th field="productName" width="200">产品名称</th>
	...
	<div id="dlg" class="easyui-dialog" style="width:600px;height:400px;padding:10px 20px"
        ...
        <form id="fm" method="post" novalidate enctype="multipart/form-data">
          <div class="fitem">
                <label>产品分类 :</label>
                <input id="product_classification_id" name="product_classification_id" class="easyui-validatebox"  style="width:100%">
            </div>
            <div class="fitem">
                <label>产品名称 :</label>
                <input id="product_name" name="product_name" class="easyui-validatebox" required="true" style="width:100%">
            </div>
            <div class="fitem">
                <label>描述:</label>
            ...
            <input type="hidden" id="product_classification_zw" name="product_classification_zw">
        ...
        <form id="fm1" method="post" novalidate enctype="multipart/form-data">
             <div class="fitem">
                ...
                <input id="lb_id" name="lb_id" class="easyui-validatebox" style="width:300px">
           		<input type="hidden" id="lb_zw" name="lb_zw">
            </div>
            <div class="fitem">
                <label>产品名称 :</label>
                <input name="productName" class="easyui-validatebox" required="true" style="width:300px">
            </div>
            <div class="fitem">
                <label>描述:</label>
            	<input name="productDescription" class="easyui-textbox" required="true" data-options="multiline:true" value="" style="width:100%;height:100px">
            </div>
...
function newProduct(){
    ...
    $('#fm').form('clear');
    $('#product_classification_id').combobox({
        url:'<%=contextPath %>/getClassification.do',
        valueField:'code',
        textField:'detail',
        onSelect:function(record){
        	$('#product_classification_id').next().children("input[type='text']").removeClass('selectInValid');
        	$("#product_classification_zw").val(record.detail);
        }
    });
}
function editProduct(){
    ...
    if (row){
        ...
        $("#lb_zw").val(row.lbZw);
        $('#lb_id').combobox({
            url:'<%=contextPath %>/get-classification-edit.do?productId='+row.id,
            valueField:'code',
            textField:'detail',
            onSelect:function(record){
...
function newProductSubmit(){
	...
	$('#product_classification_id').next().children("input[type='text']").addClass('selectInValid');
	$('#product_classification_id').next().tooltip({
	    position: 'right',
	...
	if(flag&&($('#product_classification_id').combobox('getValue')!="")){
...
function ImgManage(){
	var row = $('#productList').datagrid('getSelected');
	if(row){
		var url = '<%=contextPath%>/advertising-platform/product-image-list.jsp?id='+row.id;
