...
<body>
	 <div id="toolbar">
        ...
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="destroyUser()">删除用户</a>
         <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addRole()">分配角色</a>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="manageUserRole()">编辑用户角色关联</a>
    </div>
	<table id="userList" class="easyui-datagrid" 
			url="<%=contextPath %>/userList.json"
			title="用户管理" i
	...
	<div id="dlg" class="easyui-dialog" style="width:500px;height:200px;padding:10px 20px"
            closed="true" buttons="#dlg-buttons" title="用户管理">
        <form id="fm" method="post" novalidate>
            <div class="fitem">
                <label>用户账号 :</label>
            	<input id="userid" name="userid" value="" class="easyui-validatebox" required="true"  validType="username" style="width:200px">
            </div>
            <div class="fitem">
                <label>用户昵称 :</label>
            	<input id="username" name="username" class="easyui-validatebox" required="true"  value="" style="width:200px">
            </div>
             <div class="fitem">
                <label>手机号:</label>
            	<input id="mobile" name="mobile" class="easyui-validatebox" required="true" validType="mobile" value="" style="width:200px">
            </div>
        </form>
    </div>
    <div id="dlg-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveUserSubmit()">保存</a>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg').dialog('close')">取消</a>
    </div>
    <div id="dlg1" class="easyui-dialog" style="width:500px;height:200px;padding:10px 20px"
            closed="true" buttons="#dlg-buttons1" title="用户管理">
    ...
    <div id="dlg-buttons1">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="editUserSubmit()">保存</a>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg1').dialog('close')">取消</a>
    </div>
    <div id="dlg2" class="easyui-dialog" style="width:600px;height:150px;padding:10px 20px"
            closed="true" buttons="#dlg-buttons2" title="用户角色管理">
       	 <form id="fm2" method="post" novalidate>
            <div class="fitem">
                <label>用户账号 :</label>
            	<input id="userName" name="userName" value="" readonly="readonly"  class="easyui-validatebox"  style="width:200px">
            </div>
            <div class="fitem">
                <label>角色 :</label>
            	<input id="roleId" name="roleId" class="easyui-validatebox"  value="" style="width:200px">
            </div>
    ...
    <div id="dlg-buttons2">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="addRoleSubmit()">保存</a>
...
function saveUserSubmit(){
	url = '<%=contextPath%>/userCreate.json';
...
function destroyUser(){
    ...
    if (row){
        $.messager.confirm('删除用户','确定要删除么？',function(r){
            if (r){
                $.post('<%=contextPath%>/userDelete.json',{id:row.id},function(json){
                    if (json.result){
...
function addRole(){
	var row = $('#userList').datagrid('getSelected');
	 if (row){
		 $('#roleId').combobox({
		        url:'<%=contextPath %>/getAllRoles.do',
		        valueField:'code',
		        textField:'detail',
...
function addRoleSubmit(){
	...
	var url = '<%=contextPath%>/roleGrantUser.json';
...
function manageUserRole(){
	var row = $('#userList').datagrid('getSelected');
	if(row){
		window.location.href="<%=contextPath%>/framework/ums/user/user-role-list.jsp?userId="+row.id;
	}else{
		$.messager.show({    // show error message
	         title: '提示',
	         msg: "请先选定一行"
